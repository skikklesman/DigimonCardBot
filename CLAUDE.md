...
> **Owner email + Cloudflare account ID** are kept out of this public-bound file;
> they live in the gitignored **`CLAUDE.local.md`** on the dev machine, and the
> placeholders below point there. The **Worker URL and D1 database ID are
> deliberately public** (owner call 2026-07-15): the URL is a live endpoint
> hardcoded in `scripts/smoke.ts` + `scripts/rollback-playbook.ps1`, and the D1
> id is required in `wrangler.toml` by design. Neither is a secret.

...
3. **Failures are visible**: sync-failure and stale-sync alerts fire to the
   private webhook, and this has been *proven* by a forced failure test.
...
named in the product goal (HANDOFF §1). *(Flagged as a human decision — demote
`/alt` to Phase 4 if you disagree.)*
...
- [x] **0.1 — Repo & toolchain init.** *(git init + GitHub remote already done
  2026-07-03; toolchain landed 2026-07-04.)* TypeScript (strict) + Wrangler project; Vitest with
...
- [ ] **0.2 — CI skeleton.** GitHub Actions (or equivalent): typecheck + lint +
  ...
  *DoD: a pushed commit shows a green check.*
- [ ] **0.3 — Ed25519 verification module.** Pure function:
  ...
  *DoD: tests cover valid sig, bad sig, tampered body, missing headers.*
...
passes); CI green; signature tests in place. **Reached: ____-__-__**
...
- [ ] **1.1 — D1 + schema migrations.** `wrangler d1 create`, schema from
  [HANDOFF §5](../HANDOFF.md) as a migration file, seed `meta` with
  `active_version = 0`. Local D1 works under `wrangler dev` and vitest.
...
- [ ] **1.6 — `scheduled()` handler + first real sync.** Wire fetch → validate →
  ...
  source. *(Gate per HANDOFF §13.3: cards table holds a full versioned
  dataset — spot-check row count and a few known cards.)*
...
the test guild. **Reached: ____-__-__**
...
- [ ] **3.5 — Post-deploy smoke suite.** Scripted signed synthetic interactions
  against the *live* endpoint: PING, `/card` by ID, autocomplete query. Runs in
...
- [ ] **5.3 — Submit Discord bot verification** *(human, government ID)*

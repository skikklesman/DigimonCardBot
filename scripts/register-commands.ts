...
//
//   npm run register            # guild commands (instant, for the test guild)
...
const url = isGlobal
  ...
  : `${API}/applications/${appId}/guilds/${required("DISCORD_TEST_GUILD_ID")}/commands`;

console.log(
  `Registering ${COMMAND_DEFINITIONS.length} command(s) ${isGlobal ? "GLOBALLY (allow ~1h to propagate)" : "to the test guild"}…`,
);

// Alert module unit tests (chunk 3.3; relocated from sync/ in 4.5). The
// cardinal property: sendAlert never throws, whatever the webhook does.
...
describe("sendAlert", () => {
  it("POSTs the message as Discord webhook JSON", async () => {
    ...
    const ok = await sendAlert("https://hooks.example/x", "❌ sync failed", {
      fetchImpl: stub,
    });
    expect(ok).toBe(true);
  ...
  it("returns false without throwing when the webhook is unset", async () => {
    await expect(sendAlert(undefined, "lost alert")).resolves.toBe(false);
  });
  ...
  it("returns false without throwing on a webhook error status", async () => {
    ...
    await expect(sendAlert("https://hooks.example/x", "x", { fetchImpl: stub })).resolves.toBe(
      false,
    );
  });
  ...
  it("truncates content to Discord's 2000-char cap", async () => {
    ...
    await sendAlert("https://hooks.example/x", "y".repeat(5000), { fetchImpl: stub });

// /release tests (chunk 4.2): dataset integrity + handler resolution +
// static autocomplete. The repo is stubbed — countSetCards has its own
// integration tests in data/repo.test.ts.
...
import { createReleaseAutocomplete, createReleaseCommand } from "./release.ts";
...
const invoke = (set: string, repo = stubRepo().repo) =>
  createReleaseCommand(repo)({
    type: 2,
    data: { name: "release", options: [{ name: "set", type: 3, value: set }] },
  } as never);

const suggest = (typed: string) =>
  createReleaseAutocomplete()({
    type: 4,
    data: { name: "release", options: [{ name: "set", type: 3, value: typed, focused: true }] },
  } as never);
